...
frame_repeat = 12
resolution_k = 30
resolution = (2*resolution_k, 3*resolution_k)
# resolution = (30, 45)
...
DEFAULT_CONFIG = "../../../reference/D3_battle.cfg"
...
def create_network(session, available_actions_count):
    ...
    conv1 = tf.contrib.layers.convolution2d(s1_, num_outputs=16, kernel_size=[5, 5], stride=[2, 2],
                                            activation_fn=tf.nn.relu,
                                            weights_initializer=tf.contrib.layers.xavier_initializer_conv2d(),
                                            biases_initializer=tf.constant_initializer(0.1))
    conv2 = tf.contrib.layers.convolution2d(conv1, num_outputs=16, kernel_size=[3, 3], stride=[1, 1],
                                            activation_fn=tf.nn.relu,
                                            weights_initializer=tf.contrib.layers.xavier_initializer_conv2d(),
                                            biases_initializer=tf.constant_initializer(0.1))
...
def initialize_vizdoom(config_file_path):
    ...
    game = vzd.DoomGame()
    game.set_doom_scenario_path("../../../reference/D3_battle.wad")
...
if __name__ == '__main__':
    ...
    load_epoch = 598
    if load_model:
        print("Loading model from: ", "../../../model/model-"+str(load_epoch))
        saver.restore(session, "../../../model/model-"+str(load_epoch))
    else:
    ...
    if not skip_learning:
        for epoch in range(epochs):
            ...
            print("Saving the network weigths to:", DEFAULT_MODEL_SAVEFILE+str(epoch+load_epoch))
            saver.save(session, DEFAULT_MODEL_SAVEFILE+str(epoch+load_epoch))
